...
public class OcrService {
    ...
    public boolean contineMarker(BufferedImage imagine, String marker) {
        try {
            ...
            // System.out.println("Dimensiuni pagina" +
            // imagine.getWidth()+","+imagine.getHeight());
    ...
    public InfoPagina extrageInformatiiCerere(BufferedImage imagine) {
        try {
            ...
            // System.out.println("Dimensiuni pagina" +
            // imagine.getWidth()+","+imagine.getHeight());
            BufferedImage zona = imagine.getSubimage(0, AppConstants.Y_START_C, imagine.getWidth(),
                    AppConstants.Y_HEIGHT_C);
            String text = tesseract.doOCR(zona);
            // sablon pentru Dovezi de ridicare numar si data cererii
            // Matcher potrivire = cautaPatternMultiplu(text,
            // "Cerere:?\\s*(\\d+)\\s*din\\s*data\\s*(\\d{2}[.\\/-]\\d{2}[.\\/-]\\d{4})");
            ...
            // String firma = cautaPattern(text, "Firma:\\.?\\s*(.*)\\s*CUI"); // Caută "Nr.
            // 123" sau "Nr 123"
            String firma = cautaPattern(text, "Firma:\\.?\\s*(.*)\\s*Sediul"); // Caută "Nr. 123" sau "Nr 123"
            // return text.contains(marker);
            if (firma.length() > 15)
                firma = firma.substring(0, 15);
