...
public class ProcesorDocumente {
    ...
    public List<InfoPagina> proceseaza(File fisier, LogListener listener) throws Exception {
        // 1. Obținem token-ul (se face o singură dată sau la expirare)
//        String token = authService.getAccessToken();
//        System.out.println("Autentificat cu succes. Token activ.");
        List<InfoPagina> infoPagini = new ArrayList<>();
        try (PDDocument document = PDDocument.load(fisier)) {
            ...
            InfoPagina cerere = null, infoPag;

            
            for (int i = 0; i < document.getNumberOfPages(); i++) {
                
                System.out.println("procesam pagina "+i);
                listener.onLog("procesam pagina "+i);
                ...
                System.out.println("Actul  -- "+infoPag.getSPagina());
                if (infoPag.getPagina()==TipPagina.Incheiere) {
                if (!paginiCurente.isEmpty() && cerere != null) {
                ...
                cerere = new InfoPagina(infoPag);
            }
                
                    
                if (infoPag.getPagina()!=TipPagina.Altele) {
                    infoPagini.add(infoPag);
                    ...
                    // Dacă avem pagini adunate, le salvăm înainte de a începe un nou set
                }
                paginiCurente.add(i);
